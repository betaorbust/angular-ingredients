(function (window) {
	...
	*/
	...
	/**
	* Returns if a value is a promise. No official method exists, so this is just
	* a check to make sure that the value is an object and that it has a then method.
	* @param  {Multiple}  value    The value under test.
	* @return {Boolean}       If the value under test looked like a promise.
	*/
	window.isPromise = function(value){
		return _.isObject(value) && typeof(value.then) === 'function';
	};
}(window));